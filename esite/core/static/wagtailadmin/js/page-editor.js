    ...
    self.initChildControls = function (prefix) {
        ...
        if (opts.canOrder) {
            $("#" + prefix + "-move-up").on("click", function () {
                ...
                var prevChildOrderElem = prevChild.children(
                    'input[name$="-ORDER"]'
                );
            ...
            $("#" + prefix + "-move-down").on("click", function () {
                ...
                var nextChildOrderElem = nextChild.children(
                    'input[name$="-ORDER"]'
                );
...
function initCollapsibleBlocks() {
    $(".object.multi-field.collapsible").each(function () {
        ...
        if (
            $li.hasClass("collapsed") &&
            $li.find(".error-message").length == 0
        ) {
            $fieldset.hide();
        }
        ...
        if ($previewButton.data("auto-update")) {
            ...
            $form
                .on("change keyup DOMSubtreeModified", function () {
                    ...
                    autoUpdatePreviewDataTimeout = setTimeout(
                        setPreviewData,
                        1000
                    );
                })
