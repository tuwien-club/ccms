...
class ClubManagement:
    ...
    def send_mail(self, addresses:str, registration_link:str) -> None:
        ...
        html_message:str = render_to_string('registration/registration_mail_template.html', {'registration_link': registration_link})

        send_mail("Registrierung TUWien Club - NoTuSpam", f"{emailheader}\n\n{content}", addresses, "noreply@neurons.at", html_message=html_message)
    ...
    @classmethod
    def register_user(cls, matrikelnummer:str, telegram_user:object, register_res=False) -> bool:
            ...
            if (not DBIO.check_blacklist(matrikelnummer=matrikelnummer)):
                if (not DBIO.check_user_id(telegram_user.id)):
                    ...
                    if (DBIO.register(
                        ...
                        registration_link:str = f'{settings.BASE_URL}/{registration_token}'
